...
def build_default_composition(facts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "story_intent": "uneasy_observation",
        "reason": "fixed skeleton for stable hillside overlook composition",
    }
...
def refine_composition_with_llm(facts: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        system = (
            ...
            "Preserve a stable overlook composition and do not collapse subject-background distance. "
            "Use BEGIN_JSON and END_JSON."
        )
        user = f"""facts:
{json.dumps({
...
END_JSON"""
        raw = llm_generate(system, user, max_new_tokens=120)
...
def composition_to_tags(comp: Dict[str, Any]) -> List[str]:
    tags: List[str] = []
    if str(comp.get("camera_height")) == "high":
        tags += ["high angle"]
    if str(comp.get("subject_position")) == "foreground_right":
        tags += ["foreground", "right side"]
    elif str(comp.get("subject_position")) == "foreground_left":
        tags += ["foreground", "left side"]
    if str(comp.get("background_position")) == "far_below":
        tags += ["city below", "low horizon city"]
    elif str(comp.get("background_position")) == "horizon_far_back":
        tags += ["far horizon", "thin skyline"]
    if str(comp.get("space_layout")) == "medium_negative_space":
        tags += ["negative space", "clear separation"]
    elif str(comp.get("space_layout")) == "large_negative_space":
        tags += ["negative space", "empty space", "clear separation"]
    if str(comp.get("subject_orientation")) == "side_silhouette":
        tags += ["side silhouette"]
    elif str(comp.get("subject_orientation")) == "back_view":
        tags += ["from behind"]
    if str(comp.get("gaze_flow")) == "toward_background":
...
def compact_with_llm(skeleton_tags: List[str], effect_tags: List[str], seed_tags: List[str], composition_tags: List[str], composition: Dict[str, Any]) -> Tuple[List[str], Dict[str, Any]]:
    ...
    must_keep = [
        "wide shot", "high angle", "foreground",
        "negative space", "clear separation",
        "city below", "low horizon city",
    ...
    try:
        ...
        user = (
            ...
            "Do not remove high angle, foreground, negative space, clear separation, city below, low horizon city. "
